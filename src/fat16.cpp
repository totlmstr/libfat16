...
int main(int argc, char **argv) {
    FILE *f = fopen("os0.bin", "rb");
    Fat16::Image img((void*)f, [](void *userdata, void *buffer, std::uint32_t size) -> std::uint32_t {
    ...
    Fat16::Entry entry;
    for (int i = 0; i < img.boot_block.num_root_dirs; i++) {
        img.get_next_entry(entry);
        if (entry.entry.file_attributes & 0x10) {
            auto a = entry.get_filename();
            std::cout << " " << std::string(a.begin(), a.end()) << "\n";

            Fat16::Entry baby;
            img.get_first_entry_dir(entry, baby);

            for (int i = 0; i < 511; i++) {
                if (baby.entry.file_attributes & 0x20) {
                    auto c = baby.get_filename();
                    std::cout << "\t" << std::string(c.begin(), c.end()) << "\n";
                }
                
                img.get_next_entry(baby);
            }
